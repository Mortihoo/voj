        ...
        for ($i = 7; $i < $len; $i += 8) {
            if (strip_tags($matches[0][$i]) == HDU_USER_NAME) {
                ...
                if (mysqli_num_rows($datatem) == 0) {
                    ...
                    $len = count($matches[0]);
                    for ($k = 7; $k < $len; $k += 8) {
                        if (strip_tags($matches[0][$k]) == HDU_USER_NAME) {
                            $hduid = strip_tags($matches[0][$k - 7]);
                            $query = "select * from PostList where hduid = '" . $hduid . "'";
                            $datatem = mysqli_query($con, $query);
                            if (mysqli_num_rows($datatem) == 0) {
                                mysqli_free_result($datatem);
                                $query = "update PostList set posttime = '" . strip_tags($matches[0][$k - 6]) . "',
				status = '" . strip_tags($matches[0][$k - 5]) . "', hduid = '" . strip_tags($matches[0][$k - 7]) . "',
				runtime = '" . strip_tags($matches[0][$k - 3]) . "', memory = '" . strip_tags($matches[0][$k - 2]) . "'
			where rid = '$rid'";
                                mysqli_query($con, $query);
                                break;
                            } else
